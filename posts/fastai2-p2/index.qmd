...
# Training loop with `parameters` and `optim`

In a PyTorch module, the layers are stored inside `named_children()` attribute and the parameters are stored inside `parameters()` attribute.