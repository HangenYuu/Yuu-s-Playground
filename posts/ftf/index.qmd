...
## Data Engineer:
As a [LinkedIn](https://www.linkedin.com/feed/update/urn:li:activity:6991442210093219840/) post suggested, a data engineer's job is making sure that data analysts and data scientists can do their job. He is in charge of the data pipeline. Specifically (but not exhaustively):

1. Storing the data. It means you need a database, and when you talk about database, you talk about SQL. But wait, maybe the data need to be shareable to other teams. It means the database must be in the cloud.
...
2. Cleaning the data. The purposes are twofold: you need to get used to the data, and you need to prepare it for subsequent steps involving the models. Working with tabular data, you will want to comfortably work with Pandas and Matplotlib, but sometimes you will go a level lower with Numpy. In computer vision, you will likely hear about data augmentation, which is often within a framework like TensorFlow or PyTorch.
3. Feature engineering. ML 101: our chief task is to find the relationship between the inputs (features) and the ouputs. In simple cases, the features are just the data themselves, for example, feeding height an weight to a model to predict whether a person is obese or not. In feature engineering, we combine the base features to create new feature, for example, we calculate BMI from height and weight and feed into the model as additional inputs. We engineer features when we suspect that the additional features can lead the model to better prediction. The suspect may come from current knowledge, as with BMI and obesity, from "scientific/engineering sense" or just testing random ideas. Pandas is often something you should know.
4. Model development. First, choose a model. For most of us, it means that you need to read around to see if anyone has tried any kind of model in this problem before and how successful he was. Most likely, the answer would be "Yes" and "State-of-the-art". If no, then you will need to look for successful cases close enough to yours and adapt the model to your situation. If you are really good, you may even create new model architectures. Anyway, it all starts with the ability to research. Second, you need to train and test the model. You will use a framework for this, whether it is scikit-learn, TensorFlow, or PyTorch.
Note the loop in the diagram: you do not just create a good model and then you are done. Model development is likely to be an interative process: you train your model, your model fails spectacularly, you go back, diagnose the problem, build and train a new model before it fails... You get what I mean. Even in the (very rare) case that your model works immediately, you need to look around and try other modethods and compare them with your first methods.
Here, besides the need to know at least one framework, you also need to know about ML knowledge. The first one is *metrics* - measures of model performances such as accuracy, F1 score, etc. The second one is *diagnosis tools* e.g. confusion matrix for classification problem.
5. Productionizing the model.

![]{Screenshot 2022-12-02 152917.png}

Okay hear me out. We have come to a controversial topic: data science vs 